...
echo ""
last_version=$(curl -Ls "https://gh-proxy.com/https://api.github.com/repos//3x-ui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
# 获取 x-ui 版本
...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        last_version=$(curl -Ls "https://gh-proxy.com/https://api.github.com/repos/yimiha/3x-ui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        ...
        echo ""
        sleep 2
        wget -N --no-check-certificate -O /usr/local/x-ui-linux-$(arch).tar.gz https://gh-proxy.com/https://github.com/yimiha/3x-ui/releases/download/${last_version}/x-ui-linux-$(arch).tar.gz
        if [[ $? -ne 0 ]]; then
    ...
    wget --no-check-certificate -O /usr/bin/x-ui https://gh-proxy.com/https://raw.githubusercontent.com/yimiha/3x-ui/main/x-ui.sh
...
echo -e "----------------------------------------------"
echo -e "${green}〔3X-UI〕优化版项目地址：${yellow}https://github.com/yimiha/3x-ui${plain}"
